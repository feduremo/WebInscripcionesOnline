...
switch($accion)
{
    case 'eliminar':
        ...
        if($eliminados!=''){
            ...
            {
                $cont++;
                // Eliminamos la novedad
                $retorno = $Novedad->delete($eliminados[$i]);
                if($retorno==false)
                {
                    $error = $error&&true;
    ...
    case 'activar':
        ...
        if($activados!=''){
            ...
            {
                $cont++;
                // Activamos la novedad
                $data['activo'] = 1;
                $retorno = $Novedad->update($data, $activados[$i]);
                if($retorno==false)
                {
                    $error = $error&&true;
                    $success = $success&&false;
    ...
    case 'desactivar':
        ...
        if($desactivados!=''){
            for($i=0; $i<count($desactivados);$i++)
            {
                ...
                // Activamos la novedad
                $data['activo'] = 0;
                $retorno = $Novedad->update($data, $desactivados[$i]);
                if($retorno==false)
                {
                    $error = $error&&true;
                    $success = $success&&false;
...
if(_post('search') != ''){
    $search = _post('search');
     if(trim($search)!=""){
        $adicional_sql = '';
                $adicional_sql .= " lower(titulo) like lower('%".$search."%')";
     }
...
$resultados = $Novedad->get_list($cantidad, $pagina,$where,$ordenamiento);
...
$(function() {
  $('#confirm-actuator').click(function() {
    Boxy.confirm("Los elementos seleccionados se eliminarán por completo. <br/> ¿Desea continuar con la acción?", function() { eliminar('form_listado'); }, {title: 'Confirmar: Eliminar los elementos seleccionados.'});
    return false;
  });
});
...
<div class="box">
    ...
    <div class="table">
      ...
      <ul>
       <li>
          <div class="link-nuevo"><a href="adm.php?mod=novedad">Nueva</a></div>
       </li>
      </ul>
       <br />
                ...
                <tbody>
                            ...
                            ?>
                                    <tr class="<?php echo $class_tr; ?>">
                                        ...
                                        <td><a href="adm.php?mod=novedad&id=<?php echo $row['novedad_id']; ?>"><img src="images/modificar.gif" alt="Modificar"></a></td>
                                        <td><input type="checkbox" name="eliminados[]" id="eliminados[]" value="<?php echo $row['novedad_id']; ?>"/> </td>
...
?>
                                <tr>
                                    <td colspan="5">No se encontraron registros. Puede crear uno nuevo <a href="adm.php?mod=novedad" class="blue">aqu&iacute;</a>.</td>
                                </tr>
<?php } ?>
