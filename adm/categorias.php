    ...
    case 'eliminar':
        ...
        if ($eliminados != '') {
            for ($i = 0; $i < count($eliminados); $i++) {
                ...
                $retorno = $Categoria->delete($eliminados[$i]);
                if ($retorno == false) {
                    $error = $error && true;
    ...
    case 'activar':
        ...
        if ($activados != '') {
            for ($i = 0; $i < count($activados); $i++) {
                ...
                $data['activo'] = 1;
                $retorno = $Categoria->update($data, $activados[$i]);
                if ($retorno == false) {
                    $error = $error && true;
                    $success = $success && false;
                } else {
...
$resultados = $Categoria->get_list($cantidad, $pagina, $where, $ordenamiento);
?>
...
<div class="box">
    ...
    <div class="table">
        <form id="form_listado" name="form_listado" method="post" >
            <ul>
                <li>
                    <div class="link-nuevo"><a href="adm.php?mod=categoria">Nueva</a></div>
                </li>
            </ul>
            <br />
                    ...
                    if ($count > 0) {
                        while ($row = db::fetch_array($resultados)) {
                            ...
                            <tr class="<?php echo $class_tr; ?>">
                                ...
                                <td ><input type="text" style="width:40px" name="orden_<?php echo $row['categoria_id']; ?>" id="orden_<?php echo $row['categoria_id']; ?>" value="<?php echo $row['orden']; ?>" /></td>
                                <td ><?php if ($row['activo'] == '1') { ?><img src="images/si.gif" alt="Si" /><?php } else { ?><img src="images/no.gif" alt="No" /><?php } ?></td>
                                <td><a href="adm.php?mod=categoria&id=<?php echo $row['categoria_id']; ?>"><img src="images/modificar.gif" alt="Modificar"></a></td>
                                <td><input class="selected last" type="checkbox" name="eliminados[]" id="eliminados[]" value="<?php echo $row['categoria_id']; ?>"/> </td>
                        ...
                        ?>
                        <tr>
                            <td colspan="5">No se encontraron registros. Puede crear uno nuevo <a href="adm.php?mod=categoria" class="blue">aqu&iacute;</a>.</td>
                        </tr>
                    <?php } ?>
            ...
            <div class="pagination pagination-left">
                ...
                <ul class="pager">
                    ...
                    for ($i = 1; $i <= $cant_paginas; $i++) {
                        ?>
                        <li value="<?php echo $i; ?>" class="<?php
                    if ($pagina == $i) {
                        echo "current";
                    }
                        ?>" onclick="paginado(<?php echo $i; ?>)"><?php if ($pagina != $i) { ?> <a href="#"><?php echo $i; ?></a><?php
                    } else {
                        echo $i;
                    }
                        ?></li>
