    ...
    case 'eliminar':
        ...
        if($eliminados!=''){
            for($i=0; $i<count($eliminados);$i++)
            {
                ...
                $retorno = $Permiso->delete($eliminados[$i]);
                if($retorno==false)
                {
                    $error = $error&&true;
    ...
    case 'activar':
        ...
        if($activados!=''){
            for($i=0; $i<count($activados);$i++)
            {
                ...
                $data['activo'] = 1;
                $retorno = $Permiso->update($data, $activados[$i]);
                if($retorno==false)
                {
                    $error = $error&&true;
                    $success = $success&&false;
    ...
    case 'desactivar':
        ...
        if($desactivados!=''){
            for($i=0; $i<count($desactivados);$i++)
            {
                ...
                // Activamos la usuario
                $data['activo'] = 0;
                $retorno = $Permiso->update($data, $desactivados[$i]);
                if($retorno==false)
                {
                    $error = $error&&true;
                    $success = $success&&false;
...
$resultados = $Permiso->get_list($cantidad, $pagina,$where,$ordenamiento);
...
<div class="box">
    ...
    <div class="table">
        <form id="form_listado" name="form_listado" method="post" >
            <ul >
                <li>
                    <div class="link-nuevo"><a href="adm.php?mod=permiso">Nuevo</a></div>
                </li>
            </ul>
            <br />
                ...
                <tbody>
                            ...
                            ?>
                                    <tr class="<?php echo $class_tr; ?>">
                                        ...
                                        <td> <a href="adm.php?mod=permiso&id=<?php echo $row['permiso_id']; ?>"><img src="images/modificar.gif" alt="Modificar"></a></td>
                                        <td><input type="checkbox" name="eliminados[]" id="eliminados[]" value="<?php echo $row['permiso_id']; ?>"/></td>
...
?>
                                <tr>
                                    <td colspan="5">No se encontraron registros. Puede crear uno nuevo <a href="adm.php?mod=permiso" class="blue">aqu&iacute;</a>.</td>
                                </tr>
<?php } ?>
