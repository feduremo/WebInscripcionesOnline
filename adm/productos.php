    ...
    case 'eliminar':
        ...
        if($eliminados!=''){
            for($i=0; $i<count($eliminados);$i++)
            {
                ...
                $retorno = $Producto->delete($eliminados[$i]);
                if($retorno==false)
                {
                    $error = $error&&true;
    ...
    case 'activar':
        ...
        if($activados!=''){
            for($i=0; $i<count($activados);$i++)
            {
                ...
                $data['activo'] = 1;
                $retorno = $Producto->update($data, $activados[$i]);
                if($retorno==false)
                {
                    $error = $error&&true;
                    $success = $success&&false;
    ...
    case 'desactivar':
        ...
        if($desactivados!=''){
            for($i=0; $i<count($desactivados);$i++)
            {
                ...
                // Activamos el producto
                $data['activo'] = 0;
                $retorno = $Producto->update($data, $desactivados[$i]);
                if($retorno==false)
                {
                    $error = $error&&true;
                    $success = $success&&false;
    ...
    case 'reordenar':
        ...
        if($eliminados!=''){
            for($i=0; $i<count($eliminados);$i++)
            {
                ...
                $retorno = $Producto->update($data,$eliminados[$i]);
            }
        }
        break;
        ...
        if ($desactivados != '') {
            for ($i = 0; $i < count($desactivados); $i++) {
                ...
                $data['orden'] = $orden;
                $retorno = $Categoria->update($data, $desactivados[$i]);
                if ($retorno == false) {
                    $error = $error && true;
                    $success = $success && false;
                } else {
...
if($categoria_id!='' && $categoria_id!='0'){ $where = "producto_id IN (SELECT producto_id FROM producto_categoria WHERE categoria_id=$categoria_id) ";}
...
$resultados = $Producto->get_list($cantidad, $pagina,$where,$ordenamiento);
$rst_categorias = $Categoria->get_list();
?>
...
<div class="box">
					...
					<div class="table">
						<form id="form_listado" name="form_listado" method="post">
                                                    <ul>
                                                       <li>
                                                          <div class="link-nuevo"><a href="adm.php?mod=producto">Nuevo</a></div>
                                                       </li>
                                                      </ul>
                                                       <br />
						...
						<table>
							...
							<tbody>
                                                            <tr class="<?php echo $class_tr;?>">
                                                                ...
                                                                <?php
                                                                $cats_mostrado='';
                                                                $rst_rel = Db::query("Select categoria_id from producto_categoria where producto_id=".$row['producto_id']." limit 0,2");
                                                                while($rr = Db::fetch_array($rst_rel))
                                                                {
                                                                    if($cats_mostrado!=''){ $cats_mostrado = $cats_mostrado."...";}
                                                                    else
                                                                    {
                                                                        $rsc = $Categoria->get($rr['categoria_id']);
                                                                        $rc = $Categoria->get($rsc['categoria_padre_id']);
                                                                        if ($rc['nombre']=='') {
                                                                            $cats_mostrado = $rsc['nombre'];
                                                                        }
                                                                        else{
                                                                        $cats_mostrado = $rc['nombre']." / ".$rsc['nombre'];
                                                                        }
                                                                    }
                                                                }
                                                                if($cats_mostrado=='')
                                                                ...
                                                                <td><a href="adm.php?mod=producto&id=<?php echo $row['producto_id'];?>"><img src="images/modificar.gif" alt="Modificar"></a></td>
                                                                <td><input type="checkbox" name="eliminados[]" id="eliminados[]" value="<?php echo $row['producto_id'];?>"/> </td>
                                                            </tr>
                                                            <?php }
                                                            }else
                                                            { ?>
                                                            <tr>
                <td colspan="5">No se encontraron registros. Puede crear uno nuevo <a href="adm.php?mod=producto" class="blue">aqu&iacute;</a>.</td>
            </tr>
            <?php
            }?>
							</tbody>
